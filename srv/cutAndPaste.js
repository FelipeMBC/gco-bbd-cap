...
module.exports = cds.service.impl(async function () {
  ...
  async function createFolder(carpeta) {
    try {
      const carpet = encodeURI(carpeta);

      const tokenResponse = await getTokenValue();
      const sToken = tokenResponse.access_token;

      if (!sToken) {
        return false;
      }

      const dest = await cds.connect.to("GESTOR_DOCUMENTAL_CREATE_FOLDER");
  ...
  async function getPathSP(paths, pathspFinal, titulo) {
    ...
    `;
      ...
      for (const gpath of result) {
        ...
        const bodys = await createFolder(carpeta);
  ...
  async function updatePadre(nodoCut, nodoPaste) {
    let sql;
